...
class e2e_wm(BaseModule):
    def __init__(self, predict_frame_len, h_occ, h_occ_encodered, vqvae, transformer, pose_encoder, pose_decoder, plan_head, loss):
        super(e2e_wm, self).__init__()
        ...
        self.plan_head = HEADS.build(plan_head)
    ...
    def train_step(self, data, optimizer, **kwargs):
        ...
        #predict pose
        input_idx = ids[:, :ids.shape[1]-self.predict_frame_len].clone().detach()
        pred_pose = self.plan_head(rel_poses, input_idx, z_q_predict.clone().detach())
    ...
    def forward_test(self, data, input_times, predict_times, device='cuda'):
        ...
        for i in range(predict_times*2):
            ...
            input_idx = ids.clone().detach()
            pred_pose = self.plan_head(pose_input, input_idx, z_q_predict.clone().detach())
