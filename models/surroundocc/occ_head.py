...
class OccHead(nn.Module): 
    ...
    def forward(self, mlvl_feats, lidar2img, img_shape):
    ...
    def loss(self,
             gt_occ,
             preds_dicts):
        gt_occ = gt_occ.to(torch.float32)
        ...
        for i in range(len(preds_dicts['occ_preds'])):
            ...
            try:
                loss_occ_i = loss_occ_i + sem_scal_loss(pred, gt.long()) + geo_scal_loss(pred, gt.long())
            except:
                print('error in loss')
